...
function draw() {
    ...
    if (stage != 0 && deathCounter === 100 || stage != 0 && deathCounter > 100) {

        stage = 0;
        intro.image = loadImage("images/image0.jpeg");
    ...
    if (stage === 3) {
        ...
        fill("red");
        textSize(20);
        text("Infection Level : " + deathCounter, 400, 80);
        pop();
    }
    ...
    if (stage != 4) {
        ...
        if (stage != 0) {
            ...
            if (stage === 2) {
                ...
                text("'q' for Sanitizer Pellets", 10, 80);
...
function keyPressed() {
    ...
    if (stage === 0) {
        ...
        if (keyIsDown(83)) {
            ...
            intro.image = loadImage("images/Slide1Screen.png");
            storyVoice();
        ...
        if(keyIsDown(77)){
            ...
            intro.image = loadImage("images/Mentions.jpg");
            video.position(600, 50);
            button.hide();
            console.log("bla")
...
function level1() {
    ...
    stroke("black");
    textFont('Georgia');
    text("Maintain social distance", 550, 50);
    text("Find your path", 550, 70);
    //??level1??
    player.foward();
    player.groundC();
...
function level0() {
    ...
    if (introS === 0 && levelD === 1 && stage === 0 && deathCounter < 100) {

        intro.image = loadImage("images/Win2.jpg");
...
function storyVoice() {
    ...
    foo = new p5.Speech();
    foo.speak('This is Steve! While he already knows the complications of going outside during a pandemic, Right now he has to go to get his groceries. To most people this would be a very easy task, but for Steve, it is a battle between microbes and humans, go to the grocery store and run back home before it’s too late. Press p to start.');
